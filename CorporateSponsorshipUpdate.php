...
<h2>Update Corporate Sponsorship</h2>
<?php
				//Connect to MySQL
				...
				if (isset($_GET['intEventCorporateSponsorTypeID']))
				{
					$intEventCorporateSponsorTypeID = $_GET['intEventCorporateSponsorTypeID'];
				}
					$resultSet = $conn->query("
						...
						FROM TEventCorporateSponsorshipTypes 
							...
							JOIN TEventCorporateSponsorshipTypeBenefits ON TEventCorporateSponsorshipTypeBenefits.intEventCorporateSponsorshipTypeID = teventcorporatesponsorshiptypes.intEventCorporateSponsorTypeID
							JOIN TBenefits ON TBenefits.intBenefitID = TEventCorporateSponsorshipTypeBenefits.intBenefitID
						WHERE TEventCorporateSponsorshipTypes.intSponsorshipAvailable >= 1 AND teventcorporatesponsorshiptypes.intEventCorporateSponsorTypeID =  " .$intEventCorporateSponsorTypeID );
				
						$row = mysqli_fetch_array($result);			
					while($rows = $resultSet->fetch_assoc())
					{
						$selEvent = $rows['intEventYear'];
						$selCorporateSponsorshipType = $rows['strCorporateSponsorshipType'];
						$txtCost = $rows['decSponsorshipCost'];
						$txtSponsorshipsAvailable = $rows['intSponsorshipAvailable'];
						$selBenefits = $rows['Benefits'];
					}
				
					
?>
		<form action="UpdateCorpSponsorships.php" method="post">
			<table border="1" align=center>
				<tbody>
							...
							<select name = "selEvent">
								<?php
									while($rows = $resultSet->fetch_assoc())
									{
										...
										if($selEvent == $intEventYear)
										{
											echo "<option value='" . $rows['intEventID'] ."' selected>" . $rows['intEventYear'] ."</option>";
										}
										else
										{
						...
						<td>
							...
							<select name = "selCorporateSponsorshipType">
								<?php
									while($rows = $resultSet->fetch_assoc())
									{
										$strCorporateSponsorshipTypeRows = $rows['strCorporateSponsorshipType'];
										
									if($strCorporateSponsorshipTypeRows == $selCorporateSponsorshipType)
										{
											echo "<option value='" . $rows['intCorporateSponsorshipTypeID'] ."' selected>" . $rows['strCorporateSponsorshipType'] ."</option>";
										}
										else
										{
					...
					<tr>
						...
						<td>
							<?php echo '<input type="number" min="0.01" step="0.01" name="txtCost" value="'.$txtCost. '" size="20" maxlength="50" required>'; ?>
					...
					<tr>
						...
						<td>
							<?php echo '<input type="number" name="txtSponsorshipsAvailable" value="'.$txtSponsorshipsAvailable. '" size="20" maxlength="50" required>'; ?>
